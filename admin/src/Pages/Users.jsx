...
const Users = () => {
  ...
  const getStatusStyles = (status) => {
    switch (status.toLowerCase()) {
      case "active":
        return "bg-emerald-50 text-emerald-600 px-3 py-1 text-xs font-bold rounded-full border border-emerald-100";
      case "suspended":
        return "bg-rose-50 text-rose-600 px-3 py-1 text-xs font-bold rounded-full border border-rose-100";
      case "flagged":
        return "bg-amber-50 text-amber-600 px-3 py-1 text-xs font-bold rounded-full border border-amber-100";
      default:
        return "bg-slate-50 text-slate-500 px-3 py-1 text-xs font-bold rounded-full border border-slate-100";
    }
  };
  ...
  return (
    <div className="p-6 sm:p-10 space-y-6">
      ...
      <div className="flex justify-between items-center relative mb-6">
        <h1 className="text-[32px] font-bold text-slate-900 tracking-tight font-sans">
          User Management
        </h1>
        ...
        <div className="relative">
          <button 
            ...
            className={`flex items-center gap-2 border bg-white px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200 shadow-sm cursor-pointer ${
              selectedPlan || selectedStatus || selectedRisk 
                ...
                : "border-slate-200 text-slate-600 hover:bg-[#F8FAFC]"
            }`}
          >
          ...
          {isFilterOpen && (
            <div 
              ...
              className="absolute right-0 mt-2 w-64 bg-white border border-slate-100 rounded-2xl shadow-xl p-4 z-50 flex flex-col gap-4 text-left"
            >
              {/* Filter by Plan */}
              <div>
      ...
      {/* Users list table */}
      <div className="bg-white rounded-[24px] border border-[#E2E8F0] shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F8FAFC]/50 border-b border-[#F1F5F9]">
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider">USER</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider">PLAN</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider">STATUS</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider">RISK SCORE</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider w-[80px] max-w-[80px]">JOINED</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider text-right">ACTIONS</th>
              </tr>
            </thead>
                ...
                <tr key={user.id} className="hover:bg-slate-50/20 transition-colors">
                  <td className="px-6 py-4.5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#F1F5F9] text-[#475569] flex items-center justify-center font-bold text-sm shrink-0">
                        {user.name.charAt(0)}
                      </div>
                  ...
                  <td className="px-6 py-4.5">
                    <div className="flex items-center gap-4">
                      <div className="w-[100px] bg-[#F1F5F9] h-1.5 rounded-full overflow-hidden shrink-0">
                        <div
                          className={`h-full ${getRiskColor(user.riskScore)} rounded-full`}
                          style={{ width: `${user.riskScore}%` }}
                        />
                      </div>
                      <span className="text-slate-500 font-semibold text-xs min-w-[20px]">{user.riskScore}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4.5 text-slate-500 text-sm font-medium w-[80px] max-w-[80px] whitespace-pre-line leading-tight">
                    {user.joined.slice(0, 6) + "\n" + user.joined.slice(6)}
                  </td>
                  <td className="px-6 py-4.5 text-right relative">
                    <div className="flex items-center justify-end gap-4 text-slate-400">
                      ...
                      >
                        <FiSlash className="w-[18px] h-[18px]" />
                      </button>
                      <button 
